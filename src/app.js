...
function vecForToken(token, kind = 'q', head = 1) {
  const headOffset = head * 11;
  const base = kind === 'q' ? 3 : kind === 'k' ? 5 : 7;
  return [0, 1, 2, 3].map(i => +((pseudoValue(token, base + i + headOffset) * 2 - 1).toFixed(2)));
}
...
function renderAttention(tokens, mode) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < n; j++) {
      ...
      cells.push(`<div class="cell" style="background:hsl(210 70% ${light}%)">${w}</div>`);
    }
...
function renderQKV(tokens) {
  if (!tokens.length) {
    ...
    el.kVec.textContent = '';
    el.vVec.textContent = '';
    el.softmaxBars.innerHTML = '';
    return;
  }
  updateTokenSelector(tokens);
  const qi = +el.qkvToken.value || 0;
  const head = +(el.headSelect?.value || 1);
  const qTok = tokens[qi];
  const q = vecForToken(qTok, 'q', head);
  const kAll = tokens.map(t => vecForToken(t, 'k', head));
  const vAll = tokens.map(t => vecForToken(t, 'v', head));
  const scores = kAll.map(k => dot(q, k));
  const probs = softmax(scores);

  el.qVec.textContent = `Head ${head}\n${qTok}\n[${q.join(', ')}]`;
...
el.headSelect?.addEventListener('change', () => renderQKV(tokenize(el.prompt.value.trim())));
